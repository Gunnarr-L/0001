...
const verificaMoedaSelecionada = () => {
    ...
    if (selectMoedaValor == 'real') {
        if (opc == 'dolar'){
            ...
            textoRetorno = `Selecione uma moeda diferente para converter!`
        }
    } else if (selectMoedaValor == 'dolar') {
        if (opc == 'real'){
            textoRetorno = `
                Valor em Real: ${converteDolarEuro(inputValor.value)}
            `
        } else if (opc == 'euro'){
            textoRetorno = `
...
function exibeNome() {
    ...
    if(inputNome.value == '' || inputValor.value == '' || selectMoedaValor == 'Selecione a moeda') {
        ...
        document.getElementById("txtResult").innerHTML="Preencha todos os campos!"
    }
    else {
        // alert(`
        ...
        // `); 
        document.getElementById("txtResult").innerHTML=`Olá. ${inputNome.value},${verificaMoedaSelecionada()}`
    }
}
